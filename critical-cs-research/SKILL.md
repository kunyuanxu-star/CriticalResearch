...
若用户指定了输出目录，文件写入该目录；否则写入当前工作目录下的 `research-output/` 目录。写入完成后向用户报告文件路径。
...
所有文件写入同一输出目录。每次 Re-search 轮次更新文件时，保留历史内容（追加而非覆盖）。写入完成后向用户报告文件路径和变更摘要。
...
1. 呈现本轮迭代的变更摘要（新增/更新证据、主张状态变更、批判解决情况、饱和度变化）。
2. 显式询问用户：
