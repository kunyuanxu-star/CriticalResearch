...
run_test "Rejects string round_id" \
    bash -c "source '$PROJECT_ROOT/scripts/cr-common.sh' && cr_validate_schema '$TMP' '$PROJECT_ROOT/schemas/round.schema.json' && false || true"
rm -f "$TMP"

TMP2=$(mktemp)
echo '{"patch_id":"PP-001","linked_critique":"CRT-001","linked_round":"round-001","severity":"high","patch_type":["weaken_claim"],"affected_anchors":["abstract"],"proposed_change":{"before":"x","after":"y","rationale":"z"},"lifecycle_status":"proposed","knowledge_implication":{"literature_updates":[],"thinking_candidates":[{"rule":"test","category":"writing_rule"}]}}' > "$TMP2"
run_test "Rejects patch without experiment_obligations field" \
    bash -c "source '$PROJECT_ROOT/scripts/cr-common.sh' && cr_validate_schema '$TMP2' '$PROJECT_ROOT/schemas/paper_patch.schema.json' && false || true"
rm -f "$TMP2"
