...
## Mission
Formalize user intent into a signed Round Contract. This stage combines paper state snapshot, knowledge loading, objective definition, and full-paper coverage freeze into a single coherent contract. No research may proceed without a signed contract.
...
## Allowed Actions
- Read paper draft, claim ledger, and knowledge cards.
...
### 1. Paper State Reconstruction
Read `writing/paper-draft.md` and `state/claim-ledger.yaml`.
Extract:
...
### 2. Knowledge Loading
Read `workspace:_cr/knowledge/` cards relevant to the paper's domain.
...
From the user's objective and the paper state, define:
- `target`: concrete, bound to a claim or section, >= 10 chars.
- `scope.sections`: all paper sections (objective is weighting lens, not scope limiter).
