...
## Mission
Apply patches to the paper draft according to the patch plan and writing plan. Record every change in writing-diff.yaml. Produce patch-trace.yaml for full traceability: patch_id → critique_id → revision_decision → writing_pattern → status. Generate experiment obligations for claim-level patches. Validate patch application.

This stage enforces **Inv4**: Every paper patch must trace back to a critique, revision decision, or writing strategy.
...
## Outputs
- Updated `project:writing/paper-draft.md`
